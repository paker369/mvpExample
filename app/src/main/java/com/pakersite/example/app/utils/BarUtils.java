...
public final class BarUtils {
    ...
    public static void setStatusBarAlpha1(@NonNull final Activity activity,
                                          @IntRange(from = 0, to = 255) final int alpha,
                                          final boolean isDecor,boolean isWhite) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        hideColorView(activity);
        transparentStatusBar(activity,isWhite);
        addStatusBarAlpha(activity, alpha, isDecor);
    ...
    private static void transparentStatusBar(final Activity activity, boolean isWhite) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
